...
Rev2.0.2:   add 4 variable and 7 spare variable into coding
Rev2.0.3:   add jump funciton in Mithril. random loop to jump
Rev2.0.5:   add different timer for 4 level Mithril
            add 4 timer setting.
Rev2.0.6:   add timer between jump and right click
Rev2.0.7:   add garden function 
Rev2.0.8:   add auto move mouse and check in plot by 5 sec. 
Rev2.0.9:   add 3sec delay to move mouse & back garden direct flight to plot. 
"""
VersionD = "Rev_2.0.9"
...
EndDateString = '2025-09-08 11:59:58'
...
MithrilTime: float = 0     #
MithrilTime_2: float = 0
MithrilTime_3: float = 0
MithrilTime_4: float = 0
...
PathGard: str = r'.\Image\Garden.jpg'
PathPlot: str = r'.\Image\Plot.jpg'

PathList = [PathMult, PathHypi, PathJoin, PathCanc, PathGame, PathSkyb, PathDone, PathServ]
# PathAllList = [PathMult, PathHypi, PathJoin, PathCanc, PathGame,
# PathSkyb, PathDone, PathServ, PathIsla, PathVill, PathBack]
PathAllList = [PathMult, PathHypi, PathJoin, PathCanc, PathGame,
               PathSkyb, PathDone, PathServ, PathIsla, PathVill,
               PathBack, PathTick, PathMith, PathGard, PathPlot]
...
def move_back(time_input):
    pyautogui.keyDown('S')
    time.sleep(time_input)
    pyautogui.keyUp('S')


def move_forward(time_input):
    pyautogui.keyDown('W')
    time.sleep(time_input)
    pyautogui.keyUp('W')


def move_left(time_input):
    pyautogui.keyDown('A')
    framing_right(time_input)
    pyautogui.keyUp('A')


def move_right(time_input):
    pyautogui.keyDown('D')
    time.sleep(time_input)
    pyautogui.keyUp('D')
...
class mouse_little_move(QThread):
    def __init__(self):
        # 初始化函数
        super(mouse_little_move, self).__init__()

    def run(self):
        time.sleep(3)
        while True:
            list_max = random.randint(5, 10)
            move_list = []
            sum_list = 0
            for each in range(list_max):
                move_list.append(random.randint(-5, 5))
            for each in move_list:
                sum_list += each
            move_list.append(0-sum_list)
            for each in move_list:
                pyautogui.moveRel(0, each)
                time.sleep(random.randint(5, 100)*0.1)


class garden_checking(QThread):
    def __init__(self):
        super(garden_checking, self).__init__()

    def run(self):
        ImgPlot = LoadWBImage(PathPlot)
        time.sleep(10)
        while True:
            ImageNew = GetWBImage(PicNewList)
            # cv2.imwrite(".\ImageTemp\ImageMith" + str(PollingMithril) + ".jpg", ImageNew)
            if not haveSmallImage(ImageNew, ImgPlot):
                pyautogui.keyUp('A')
                pyautogui.keyUp('D')
                pyautogui.mouseUp(button='left')
                backPIsland()
...
class startGetStoneThread(QThread):
    ...
    def run(self):
        ...
        global Spare_3
        global Spare_4
        global MithrilTime_2
        global MithrilTime_3
        global MithrilTime_4
        ...
        ImgGarden = LoadWBImage(PathGard)
        ImgPlot = LoadWBImage(PathPlot)
        ...
        while True:
            ...
            if logFlag or not loggedFlag:
                ...
                for each in PathList:
                    PollingSerPic += 1
                    if hypixelFlag and each == PathHypi:
                        hypixelFlag = False
                        DataLog(self.LogFileName, 'hypixelFlag set to 0')
                        continue
                    if gameMenuFlag and each == PathGame:
                        gameMenuFlag = False
                    ...
                    if ScreenImg_ is None:
                        ...
                        if each == PathSkyb:
                            ...
                            if haveSmallImage(screenPartImg, ImgGarden):
                                loggedFlag = True
                                logFlag = False
                                DataLog(self.LogFileName, 'logged, and in Garden')
                                backPIsland()
                ...
                if self.startFromHUBFlag == 1:
                    ...
                    while True:
                        time.sleep(LogPicTakeDelay)
                        ImageNew = GetWBImage(PicNewList)
                        if haveSmallImage(ImageNew, ImgGarden):
                            break
                        back_to_garden()
                    garden_to_plot()
                ...
                putChestTime = 1
                PollingMithril = 0
                JumpLoopControl = random.randint(10, 15)
                ...
                while True:
                    ...
                    for each in range(TpStart - 1, int(TpMax / 2)):
                        PollingMithril += 1
                        framing_left(pressTimeInput)
                        little_right()

                        ImageNew = GetWBImage(PicNewList)
                        cv2.imwrite(".\ImageTemp\ImageMith" + str(PollingMithril) + ".jpg", ImageNew)
                        if not haveSmallImage(ImageNew, ImgPlot):
                            DataLog(self.LogFileName, 'Not in garden')
                            # print("not in garden")
                            break
                        print('yes. in garden. ready to right')

                        move_back(afterTime)
                        little_forward()
                        framing_right(pressTimeInput)
                        little_left()
                        ...
                        if not haveSmallImage(ImageNew, ImgPlot):
                            ...
                            # print("not in garden")
                            break
                        print('yes. in garden. ready to left')
                    ...
                    ImageNew = GetWBImage(PicNewList)
                    cv2.imwrite(".\ImageTemp\ImageMith" + str(PollingMithril) + ".jpg", ImageNew)
                    if haveSmallImage(ImageNew, ImgPlot):
                        pass
                    elif haveSmallImage(ImageNew, ImgIsland):
                        self.startFromHUBFlag = 1
                        DataLog(self.LogFileName, "now is in island")
                        break
                    elif haveSmallImage(ImageNew, ImgVillage):
                        self.startFromHUBFlag = 1
                        DataLog(self.LogFileName, "now is in HUB")
                        break
                    else:
                    ...
                    #         if each[2] == 1:
                    #             time.sleep(MithrilTime)
                    #         elif each[2] == 2:
                    #             time.sleep(MithrilTime_2)
                    #         elif each[2] == 3:
                    #             time.sleep(MithrilTime_3)
                    #         elif each[2] == 4:
                    #             time.sleep(MithrilTime_4)
                    #
                    #     pyautogui.mouseUp(button="left")
                    #     pyautogui.keyUp("shift")
                    #     # pyautogui.press("space")
                    #     if PollingMithril % JumpLoopControl == 0:
                    #         JumpLoopControl = random.randint(7, 8)
                    #         pyautogui.keyDown("space")
                    #         pyautogui.keyUp("space")
                    #         time.sleep(MithrilTime_4)
...
class MainFunWindow(QMainWindow, Ui_MainWindow):
    ...
    def UiInitialData(self):
        ...
        for each in self.ConfList:
            ...
            elif each[0] == 'lineEditMithrilTime':
                MithrilTime = float(each[1])
            elif each[0] == 'lineEditTitaniumTime':
                TitaniumTime = float(each[1])
            elif each[0] == 'lineEditSkillTime':
                SkillTime = int(each[1])
            elif each[0] == 'lineEditSkillCooldown':
                SkillCooldown = int(each[1])
            elif each[0] == 'lineEditSpare':
                Spare = int(each[1])
            elif each[0] == 'lineEditSpare_2':
                Spare_2 = int(each[1])
            elif each[0] == 'lineEditSpare_3':
                Spare_3 = int(each[1])
            elif each[0] == 'lineEditSpare_4':
                Spare_4 = int(each[1])
            elif each[0] == 'lineEditMithrilTime_2':
                MithrilTime_2 = float(each[1])
            elif each[0] == 'lineEditMithrilTime_3':
                MithrilTime_3 = float(each[1])
            elif each[0] == 'lineEditMithrilTime_4':
                MithrilTime_4 = float(each[1])
        ...
        self.labelMithrilTime.setText(str(MithrilTime))
        self.labelTitaniumTime.setText(str(TitaniumTime))
        self.labelSkillTime.setText(str(SkillTime))
        self.labelSkillCooldown.setText(str(SkillCooldown))
        self.labelSpare.setText(str(Spare))
        self.labelSpare_2.setText(str(Spare_2))
        self.labelSpare_3.setText(str(Spare_3))
        self.labelSpare_4.setText(str(Spare_4))
        self.labelMithrilTime_2.setText(str(MithrilTime_2))
        self.labelMithrilTime_3.setText(str(MithrilTime_3))
        self.labelMithrilTime_4.setText(str(MithrilTime_4))
        ...
        for each in PathAllList:
            ...
            if eachName == "Mult":
                ...
                self.labelGard.setText(self.labelGard.text() + ': ' + str(TempImg[0]) + " X " + str(TempImg[1]))
                self.labelGardD.setPixmap(QPixmap(each))
        ...
        if PotatoFlag == 1:
            ...
            self.label.setText("Minecraft自动小麦脚本")
        if PutChestFlag == 1:
        ...
        self.startPicCheck = garden_checking()
        self.startMoveMouse = mouse_little_move()
    ...
    def applyData(self):
        ...
        if self.lineEditMithrilTime.text() != '':
            MithrilTime = float(self.lineEditMithrilTime.text())
            self.labelMithrilTime.setText(str(MithrilTime))
            self.lineEditMithrilTime.setText('')
        if self.lineEditTitaniumTime.text() != '':
            TitaniumTime = int(self.lineEditTitaniumTime.text())
            self.labelTitaniumTime.setText(str(TitaniumTime))
            self.lineEditTitaniumTime.setText('')
        if self.lineEditSkillTime.text() != '':
            SkillTime = int(self.lineEditSkillTime.text())
            self.labelSkillTime.setText(str(SkillTime))
            self.lineEditSkillTime.setText('')
        if self.lineEditSkillCooldown.text() != '':
            SkillCooldown = int(self.lineEditSkillCooldown.text())
            self.labelSkillCooldown.setText(str(SkillCooldown))
            self.lineEditSkillCooldown.setText('')
        if self.lineEditSpare.text() != '':
            Spare = int(self.lineEditSpare.text())
            self.labelSpare.setText(str(Spare))
            self.lineEditSpare.setText('')
        if self.lineEditSpare_2.text() != '':
            Spare_2 = int(self.lineEditSpare_2.text())
            self.labelSpare_2.setText(str(Spare_2))
            self.lineEditSpare_2.setText('')
        if self.lineEditSpare_3.text() != '':
            Spare_3 = int(self.lineEditSpare_3.text())
            self.labelSpare_3.setText(str(Spare_3))
            self.lineEditSpare_3.setText('')
        if self.lineEditSpare_4.text() != '':
            Spare_4 = int(self.lineEditSpare_4.text())
            self.labelSpare_4.setText(str(Spare_4))
            self.lineEditSpare_4.setText('')
        if self.lineEditMithrilTime_2.text() != '':
            MithrilTime_2 = float(self.lineEditMithrilTime_2.text())
            self.labelMithrilTime_2.setText(str(MithrilTime_2))
            self.lineEditMithrilTime_2.setText('')
        if self.lineEditMithrilTime_3.text() != '':
            MithrilTime_3 = float(self.lineEditMithrilTime_3.text())
            self.labelMithrilTime_3.setText(str(MithrilTime_3))
            self.lineEditMithrilTime_3.setText('')
        if self.lineEditMithrilTime_4.text() != '':
            MithrilTime_4 = float(self.lineEditMithrilTime_4.text())
            self.labelMithrilTime_4.setText(str(MithrilTime_4))
            self.lineEditMithrilTime_4.setText('')
    ...
    def saveData2File(self):
        ...
        for each in self.ConfList:
            if each[0] == 'lineEditLR':
                ...
                each[1] = str(MithrilTime)
            elif each[0] == 'lineEditTitaniumTime':
                each[1] = str(TitaniumTime)
            elif each[0] == 'lineEditSkillTime':
                each[1] = str(SkillTime)
            elif each[0] == 'lineEditSkillCooldown':
                each[1] = str(SkillCooldown)
            elif each[0] == 'lineEditSpare':
                each[1] = str(Spare)
            elif each[0] == 'lineEditSpare_2':
                each[1] = str(Spare_2)
                ...
                each[1] = str(MithrilTime_2)
            elif each[0] == 'lineEditMithrilTime_3':
                each[1] = str(MithrilTime_3)
            elif each[0] == 'lineEditMithrilTime_4':
                each[1] = str(MithrilTime_4)
    ...
    def startGetStone(self):
        ...
        self.startMoveMouse.start()
        self.startPicCheck.start()
    ...
    def stopGetStone(self):
        ...
        self.startMoveMouse.terminate()
        self.startPicCheck.terminate()
        self.pushButtonStop.setEnabled(False)
